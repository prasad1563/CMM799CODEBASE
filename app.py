...
@st.cache_resource
def load_artifacts():
    ld_model = joblib.load(os.path.join(MODEL_DIR, "rf_model.pkl"))
    ld_le = joblib.load(os.path.join(MODEL_DIR, "label_encoder.pkl"))
    ld_drr = pd.read_csv(
        os.path.join(
            MODEL_DIR,
        ...
        index_col=0,
        header=0)
    ld_drr.index.name = "District"

    with open(os.path.join(MODEL_DIR, "lstm_v2_meta.json"), "r", encoding="utf-8") as f:
        ld_meta = json.load(f)
    ld_sc_seq = joblib.load(os.path.join(MODEL_DIR, "lstm_v2_scaler_seq.pkl"))
    ld_sc_stat = joblib.load(os.path.join(MODEL_DIR, "lstm_v2_scaler_stat.pkl"))
    ...
    ld_history_df = pd.read_csv(data_path, parse_dates=["YearMonth"])

    ld_lstm_model = LeptoLSTM_v2(
        ld_meta["seq_input_dim"],
        ld_meta["static_input_dim"],
        hidden_dim=128,
        lstm_layers=2,
        dropout=0.3,
    )
    with open(os.path.join(MODEL_DIR, "lstm_v2_model.pth"), "rb") as f_model:
        ld_lstm_model.load_state_dict(
            torch.load(f_model, map_location="cpu"))
    ld_lstm_model.eval()

    return ld_model, ld_le, ld_drr, ld_meta, ld_sc_seq, ld_sc_stat, ld_history_df, ld_lstm_model
...
with tab_pred:
    ...
    with col_res:
        ...
        css_class = "risk-high" if label == "High Risk" else "risk-low" # pylint: disable=invalid-name
        icon = "⚠️" if label == "High Risk" else "✅" # pylint: disable=invalid-name
        st.markdown(
            f"""
        <div class="{css_class}">
            <h2>{icon} {label}</h2>
    ...
    with col_context:
        ...
        dr_color = "#f85149" if hist_risk_pct > 50 else "#3fb950"  # pylint: disable=invalid-name
...
with tab_about:
 ...
 #30363d; border-radius:12px; padding:28px 32px; margin-bottom:24px;">
 ...
 line-height:1.8; margin-bottom:0;">
            ...
            <strong style="color:#ffffff;">spatiotemporal early-warning\
 predictive modelling of Leptospirosis incidence in Sri Lanka</strong>.
            It translates complex epidemiological data and machine\
 learning architectures into an accessible interface for public health stakeholders.
        </p>
    </div>
    """,
        unsafe_allow_html=True,
 ...
 <div style="background:#161b22; border:1px solid #30363d; border-radius:12px; padding:28px;">
            ...
            <ul style="color:#c9d1d9; line-height:1.9; margin-bottom:24px;">
                <li><strong>Predictive Engine:</strong> A calibrated Hybrid Ensemble integrating state-of-the-art\
 Deep Learning (Bidirectional LSTM with Temporal Attention) to capture sequential memory,\
 combined with a Random Forest meta-classifier (40% weight) to handle complex cross-sectional interactions.</li>
                <li><strong>Decision Logic:</strong> Optimised at an early-warning threshold of <strong>0.45</strong>\
 to prioritise system Sensitivity (Recall), ensuring high-risk outbreak scenarios are reliably detected with minimal false negatives.</li>
                <li><strong>Explainable AI (XAI):</strong> Integrated SHAP (SHapley Additive exPlanations) visualizations\
 to provide transparent, interpretable risk drivers (such as temporal incidence lags and climatic anomalies).</li>
            </ul>
            ...
            <ul style="color:#c9d1d9; line-height:1.9; margin-bottom:0;">
                <li><strong>Methodology:</strong> Evaluated using Strict Time-Series splits with sliding window\
 cross-validation to rigorously prevent look-ahead bias and temporal data leakage.</li>
                <li><strong>Statistical Significance:</strong> The proposed ensemble was mathematically validated against\
 robust baselines using DeLong's Test, achieving a project-leading <strong>91.2% Test ROC-AUC</strong>.</li>
                <li><strong>Feature Space:</strong> Utilizes 51 distinct sociodemographic, climate, agricultural (paddy yield matrices),\
 and lagged bio-climatic characteristics spanning all 25 districts of Sri Lanka.</li>
            </ul>
            ...
            <ul style="color:#c9d1d9; line-height:1.9;">
                <li><strong>Geographical Scope:</strong> Monthly time-steps mapped across\
 all 25 administrative districts of Sri Lanka.</li>
                <li><strong>Data Volume:</strong> 5,100 total temporal observations\
 (Training Set: 3,900 samples · Held-out Test Set: 1,200 samples).</li>
                <li><strong>Dimensionality:</strong> 51 dynamic and static features (Meteorological, Soil Moisture,\
 Agricultural Census, BioClimatic indicators, and engineered temporal lags).</li>
            </ul>
        </div>
        """,
            unsafe_allow_html=True,
...
<div style="background:#161b22; border:1px solid #30363d; border-radius:12px; padding:16px; text-align:center;">
 ...
 text-transform:uppercase; letter-spacing:.07em; margin:0 0 12px;">District Incidence Map</p>
                <img src="data:image/png;base64,{img_b64}"
                     style="width:100%; height:auto; object-fit:contain;\
 border-radius:8px; display:block;"
                     alt="District-level Leptospirosis\
 incidence map — Sri Lanka" />
                <p style="color:#8b949e; font-size:0.75rem;\
